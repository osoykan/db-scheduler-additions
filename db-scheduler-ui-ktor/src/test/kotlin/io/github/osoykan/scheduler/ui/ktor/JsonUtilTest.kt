...
class JsonUtilTest : FunSpec({
  ...
  test("should serialize mixed complex and primitive types") {
    data class Config(val enabled: Boolean, val timeout: Int)
    val map = mapOf(
      "id" to 1,
      "name" to "Service",
  ...
  test("should handle deeply nested complex structures") {
    data class Metrics(val count: Int, val rate: Double)
    data class Settings(val enabled: Boolean, val metrics: Metrics)
    data class Task(val name: String, val settings: Settings, val tags: List<String>)
    ...
    json shouldBe """{"tasks":[{"name":"task-1","settings":{"enabled":true,"metrics":{"count":100,"rate":0.95}},"tags":["high-priority","critical"]},{"name":"task-2","settings":{"enabled":false,"metrics":{"count":50,"rate":0.85}},"tags":["low-priority"]}],"metadata":{"version":"1.0","timestamp":"2025-11-11T00:00:00Z"}}"""
  }
